...
function renderBeatMetrics(){
 if(!experimentalMetrics)return;
 const hrv={...beatMetrics.result,bpm:previewBeats.result.bpm,ibi:previewBeats.result.ibi};
 for(const [key,value] of Object.entries(hrv)){const el=document.getElementById('metric-'+key);if(el)el.textContent=value===null?'—':value.toFixed(2);}
 const seconds=beatMetrics.rr.length?beatMetrics.rr.at(-1).t-beatMetrics.rr[0].start:0;
 document.getElementById('beat-status').textContent=currentSqi>.38&&currentSqi<=.5 ? `Provisional beats: ${previewBeats.reason}. HRV withheld: SQI must exceed 0.50.` : beatMetrics.reason;
 document.getElementById('beat-progress').textContent=`SQI ${Number.isFinite(currentSqi)?currentSqi.toFixed(2):'unavailable'} / IBI preview > 0.38; HRV > 0.50 · ${previewBeats.rr.length} preview intervals; ${beatMetrics.rr.length} HRV intervals · ${seconds.toFixed(1)} / 60 s HRV · ${seconds.toFixed(1)} / 120 s spectral`; 
 window.facephysMetrics={hr:currentSqi>.38&&performance.now()-lastFaceDetectTime<500?lastHrValue:null,SQI:Math.max(0,Math.min(1,currentSqi)),latency:Number(document.getElementById('latencyVal')?.textContent)||null,hrv:{...hrv},beatEstimatesProvisional:true,beatPreviewSqiThreshold:0.38,hrvSqiThreshold:0.50};
}
setInterval(()=>{if(lastMetricArrival&&performance.now()-lastMetricArrival>1500){beatMetrics.reset();previewBeats.reset();beatMetrics.reason='Waiting for fresh signal';renderBeatMetrics();lastMetricArrival=0;}},500);
...
function handleInferenceResult(payload) {
    const { value, time, projOutput, timestamp, captureTimestamp } = payload;
    if(experimentalMetrics){
    lastMetricArrival=performance.now();
    const faceFresh=performance.now()-lastFaceDetectTime<500;
    previewBeats.add(value,captureTimestamp,currentSqi>SQI_THRESHOLD && faceFresh && isRunning && bufferFull);
    beatMetrics.add(value,captureTimestamp,currentSqi>.5 && faceFresh && isRunning && bufferFull);
    if(!isRunning)beatMetrics.reason='Measurement stopped';
    else if(!Number.isFinite(captureTimestamp))beatMetrics.reason='Missing video timestamps — reload this page';
    else if(!faceFresh)beatMetrics.reason='Face tracking lost — face the camera';
    else if(!bufferFull)beatMetrics.reason='Filling FacePhys signal buffer (about 15 seconds)';
...
async function startSystem() {
    beatMetrics.reset();previewBeats.reset();
    lastProcessedVideoTime=null;
    if (!measurementConsent) {
        measurementConsent = window.confirm("Start FacePhys measurement of yourself? Camera frames and physiological signals are processed on this device. Nothing is uploaded. Stop the camera to end measurement. Local exports may contain physiological data. This research model is not a medical device.");
...
let lastProcessedVideoTime = null;
function processFrame() {
    if (videoElement.readyState < 2) return;

    const sourceTime=videoElement.currentTime;
    if(sourceTime===lastProcessedVideoTime)return;
    lastProcessedVideoTime=sourceTime;
    ...
    if (detections.detections.length > 0) {
        ...
        if (sw > 0 && sh > 0) {
            ...
            inferenceWorker.postMessage({ 
                type: 'run', 
                payload: { imgData: inputFloat32, dtVal: dval, timestamp: virtualTime, captureTimestamp: videoElement.currentTime } 
            }, [inputFloat32.buffer]);
        }
    } else {
